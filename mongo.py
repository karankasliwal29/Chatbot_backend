...
def generate_chatbot_response(user_query):
    """Generate the chatbot response in a structured, readable paragraph."""
    ...
    document_text = " ".join(
        [
            " ".join([f"{key.replace('_', ' ')} is {value}." for key, value in doc.items()])
            for doc in documents
        ]
    )
    ...
    prompt = (
        ...
        f"Context from MongoDB: {document_text}\n\n"
        "Assistant: Provide a structured, well-formed paragraph response that is clear and conversational."
    )
